...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = self.attention(self.layer_norm_1(x), mask=mask)
        hidden_state += x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = causal.unsqueeze(0).expand(batch_size, -1, -1)  # -> (batch, seq, seq)

        x = self.embeddings(input_ids)  # -> (batch, seq, d_model)
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        # LM head maps d_model -> vocab_size
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.transformer_decoder(input_ids)  # (batch, seq, d_model)
